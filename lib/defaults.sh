...
# ====================
# Modifier Key Mapping Functions
# ====================
#
# macOS stores modifier key remappings in ByHost/.GlobalPreferences plist
# These settings are keyboard-specific (tied to VendorID-ProductID).
#
# Reference: Apple Technical Note TN2450
# https://developer.apple.com/library/archive/technotes/tn2450/_index.html

# Function to save modifier key mappings as reference (backup only, no auto-restore)
# Usage: save_modifier_key_mappings "output_file"
save_modifier_key_mappings() {
    local output_file="$1"

    echo "Exporting modifier key mappings..."

    # Export raw defaults output for reference
    {
        echo "# Modifier Key Mappings (Reference)"
        echo "# Generated by backup.sh"
        echo "#"
        echo "# To configure: System Settings → Keyboard → Keyboard Shortcuts → Modifier Keys"
        echo "#"
        defaults -currentHost read -globalDomain 2>/dev/null | grep -A 20 "com.apple.keyboard.modifiermapping" || echo "# No modifier key mappings configured"
    } > "$output_file"

    echo "✓ Modifier key mappings saved to $output_file"
}