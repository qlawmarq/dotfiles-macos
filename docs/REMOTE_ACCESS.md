...
電車の中で iPhone からコーディングしたい、カフェで始めた作業を帰宅後に iPhone で続けたい——そんな要望を実現するリモートアクセス環境の構築方法を紹介します。
...
macOS に接続できたら、tmux セッションを開始します、または既存のセッションに参加する事もできます：
...
このセットアップは以下のセキュリティ機能を備えています：

- **エンドツーエンド暗号化**: Tailscale による全通信の暗号化
- **ポート開放不要**: インターネットから直接アクセス不可
- **デバイス認証**: 各デバイスが個別に認証される

### 推奨設定

1. **Tailscale で 2FA 有効化**（必須）: [管理画面](https://login.tailscale.com/admin)から設定
2. **macOS に強力なパスワード設定**: ユーザーアカウントのセキュリティ強化
3. **定期的なアップデート**: すべてのツールを最新版に保つ
4. **公開鍵認証の利用**（オプション）: パスワード認証より安全
...
- **通勤・移動中**: 電車内でも Mosh の接続維持機能で快適に作業
- **長時間タスク**: ビルドやテストを tmux でバックグラウンド実行、進捗を iPhone で確認
- **デバイス間の移動**: カフェで Mac 作業 → 帰宅後 iPhone で続きを実行
- **ペアプログラミング**: 同一 tmux セッションを複数デバイスで共有
- **緊急対応**: 外出先から Termius で即座にサーバーにアクセス
